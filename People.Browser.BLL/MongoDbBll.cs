    ...
    public class MongoDbBll
    {
        ...
        public bool GetAllPersons(out List<Person> allPersons, out string result)
        {
            ...
            try
            {
                ...
                if (!mongoDbDal.Retrieve("Persons", new Dictionary<string, string>(), out records, out result))
                {
                    result = $"Failed Retrieving 'Persons'";
                ...
                if (records != null)
                {
                    numberOfPersons = records.Count;

                    Audit($"{numberOfPersons} Persons In Total", method, LINE(), AuditSeverity.Information);
                }
                else
                {
                    Audit("No Persons Retrieved", method, LINE(), AuditSeverity.Information);
                }
        ...
        public bool GetAllCountries(out Countries allCountries, out string result)
        {
            ...
            try
            {
                ...
                List<List<string>> records;
                if (!mongoDbDal.Retrieve("Countries", new Dictionary<string, string>(), out records, out result))
                {
                    result = $"Failed Retrieving 'Countries'";
                ...
                if (records != null)
                {
                    numberOfCountries = records.Count;

                    Audit($"{numberOfCountries} Countries In Total", method, LINE(), AuditSeverity.Information);
                }
                else
                {
                    Audit("No Countries Retrieved", method, LINE(), AuditSeverity.Information);
                }  
                ...
                foreach (List<string> record in records)
                {
                    ...
                    try
                    {
                        countryId = int.TryParse(record[0], out countryId) ? countryId : Constants.NONE;
                        country.Id = countryId;

                        country.Name = record[1];
                        country.NameInEnglish = record[2];
                    }
                    catch (Exception ex)
                    {
        ...
        public bool GetAllCities(out Cities allCities, out string result)
        {
            ...
            try
            {
                ...
                List<List<string>> records;
                if (!mongoDbDal.Retrieve("Cities", new Dictionary<string, string>(), out records, out result))
                {
                    result = $"Failed Retrieving 'Cities'";
                ...
                if (records != null)
                {
                    numberOfCities = records.Count;

                    Audit($"{numberOfCities} Cities In Total", method, LINE(), AuditSeverity.Information);
                }
                else
                {
                    Audit("No Cities Retrieved", method, LINE(), AuditSeverity.Information);
                }
                ...
                foreach (List<string> record in records)
                {
                    ...
                    try
                    {
                        cityId = int.TryParse(record[0], out cityId) ? cityId : Constants.NONE;
                        city.Id = cityId;
